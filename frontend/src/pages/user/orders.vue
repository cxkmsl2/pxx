<template>
  <div class="orders">
    ...
    <div class="order-card" v-for="o in filtered" :key="o.id" @click="toggleExpand(o.id)">
...
<style scoped>
.orders { background: #f5f5f5; min-height: 100vh; }
...
.tab { padding: 6px 16px; border-radius: 16px; font-size: 13px; background: #f5f5f5; color: #666; cursor: pointer; }
...
.s1, .s5 { background: #f5f5f5; color: #999; }
.s2, .s3 { background: #fff0f0; color: #ff4d4f; }
.s4 { background: #e8f8e8; color: #52c41a; }
.order-amount { font-size: 18px; font-weight: 700; color: #ff4d4f; }
...
.pay { background: #ff4d4f; color: #fff; }
